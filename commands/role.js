    ...
    message.command(false, async () => {
        ...
        if (!message.args.length){
            ...
            if (data.modules.roles.units[message.args[0].toLowerCase()]){
                if (!message.guild.roles.has(data.modules.roles.units[message.args[0].toLowerCase()])) return message.channel.send(`Role with given name \`${message.args[0].toLowerCase()}\` seems to be deleted, contact admins!`);
